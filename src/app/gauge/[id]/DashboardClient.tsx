...
interface DashboardClientProps {
  initialGaugeId?: string  // Optional - when provided, skips URL/localStorage restoration
}

export default function DashboardClient({ initialGaugeId }: DashboardClientProps) {
  // Initialize with the gauge ID from prop (if provided) or null (restored later from URL/localStorage)
  const [selectedGaugeId, setSelectedGaugeId] = useState<string | null>(initialGaugeId ?? null)
  ...
  const prevSelectedGaugeId = useRef<string | null>(initialGaugeId ?? null)

  // Restore selected gauge from localStorage on mount (only if no initialGaugeId provided)
  useEffect(() => {
    // Skip restoration if initialGaugeId was provided (e.g., from /gauge/[id] route)
    if (initialGaugeId) {
      hasMounted.current = true
      return
    }

    // Restore from localStorage for the root page
  ...
  useEffect(() => {
    ...
    if (selectedGaugeId) {
      ...
      // Update URL to /gauge/{id} path for proper OpenGraph metadata when sharing
      window.history.replaceState({}, '', `/gauge/${selectedGaugeId}`)
                  ...
                  <>
                    ...
                    {isDataStale(selectedGaugeBasic.reading.timestamp) && (
                      <div className="text-sm p-2 rounded mb-4 bg-yellow-50 text-yellow-800">
                        <span className="font-medium">Warning: Data may be outdated.</span> Last updated {formatTimeSince(selectedGaugeBasic.reading.timestamp)}
                      </div>
                    )}
